...
# STOCK 5: PFE Pfizer
# ============================================================
getSymbols("PFE", from = "1999-12-01", to = "2025-11-02", periodicity = "monthly")
...
for (year in 1:length(results_mean_list)) {
  ...
  wealth_sp500 <- sp500_monthly$compound_growth[47+year*periods_per_year]/sp500_monthly$compound_growth[47]
...
wealth_long <- wealth_data %>%
  mutate(Year = Year + 2004) %>%  # Ajouter 2004 comme année de base
  pivot_longer(cols = c("Wealth_GMVP_Gross", "Wealth_GMVP_Net", "Wealth_MV_Gross", "Wealth_MV_Net", "Wealth_sp500"),
               names_to = "Portfolio",
               values_to = "Wealth") %>%
  mutate(
    Type = case_when(
...
# ============================================================
# Combined plot: GMVP, MV, and S&P 500 on same graph
# ============================================================
# Rename for display
...
ggplot(wealth_long, aes(x = Year, y = Wealth, color = Portfolio_Display, linetype = Type)) +
  ...
  scale_x_continuous(limits = c(2004, 2026)) +  # Définir les limites des années
  labs(
...
# ============================================================
# Now we take into account inflation
# ============================================================

getSymbols("CPIAUCSL", src = "FRED", from = "2003-01-01", to = "2026-12-31", auto.assign = TRUE)
...
for (year in 1:(length(results_mean_list) - 1)) {
  ...
  cpi_indicator <- cpi_data$CPI[11 + year * periods_per_year] / cpi_data$CPI[11]
...
# Convert to long format - NOMINAL values
wealth_long <- wealth_data %>%
...
# Convert to long format - REAL values (inflation-adjusted)
wealth_long_real <- wealth_data %>%
  mutate(Year = Year + 2004) %>%
  pivot_longer(cols = c("Wealth_GMVP_Gross_Real", "Wealth_GMVP_Net_Real", "Wealth_MV_Gross_Real", "Wealth_MV_Net_Real"),
               names_to = "Portfolio",
               values_to = "Wealth") %>%
  mutate(
    ...
    Adjustment = "Real (Inflation-Adjusted)"
  )
...
# Plot GMVP: red and pink (NOMINAL)
ggplot(subset(wealth_long, Group == "GMVP"), aes(x = Year, y = Wealth, color = Type, linetype = Type)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = gmvp_colors, labels = c("Gross" = "GMVP gross", "Net" = "GMVP net")) +
  scale_linetype_manual(values = c("Gross" = "solid", "Net" = "dashed")) +
  labs(title = "Wealth GMVP: Gross vs Net (Nominal)", x = "Year", y = "Wealth (initial = 1)", color = "Type", linetype = "Type") +
  theme_minimal()

# Plot MV: blue and purple (NOMINAL)
ggplot(subset(wealth_long, Group == "MV"), aes(x = Year, y = Wealth, color = Type, linetype = Type)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = mv_colors, labels = c("Gross" = "MV gross", "Net" = "MV net")) +
  scale_linetype_manual(values = c("Gross" = "solid", "Net" = "dashed")) +
  labs(title = "Wealth MV: Gross vs Net (Nominal)", x = "Year", y = "Wealth (initial = 1)", color = "Type", linetype = "Type") +
  theme_minimal()
...
ggplot(wealth_long, aes(x = Year, y = Wealth, color = Portfolio_Display, linetype = Type)) +
  ...
  labs(
    title = "Wealth Evolution: Gross vs Net (GMVP, MV, and S&P 500) - Nominal",
    x = "Year",
    y = "Wealth (initial = 1)",
    color = "Portfolio",
    linetype = "Type"
...
# D'abord, créer wealth_long_real_with_sp500 en incluant le S&P 500
...
ggplot(wealth_long_real_with_sp500, aes(x = Year, y = Wealth, color = Portfolio_Display, linetype = Type)) +
  geom_line(size = 1.2) +
  scale_color_manual(
