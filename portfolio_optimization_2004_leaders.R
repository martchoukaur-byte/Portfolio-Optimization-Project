...
mean_rf_full <- mean(rf_data$rf_rate, na.rm = TRUE)
mean_rf_150 <- mean(rf_data$rf_rate[1:150], na.rm = TRUE)
...
for (year in 1:n_years) {
  ...
  mean_rf_window <- mean(rf_data$rf_rate[1:end_period], na.rm = TRUE)
...
for (year in 1:(length(optimization_results))) {
  ...
  rf_year <- ifelse(any(is.na(rf_subset)),
                    mean(rf_subset, na.rm = TRUE),
                    mean(rf_subset))
