...
# STOCK 2: NVDA NVIDIA
# ============================================================
getSymbols("NVDA", from = "1999-11-01", to = "2025-11-02", periodicity = "monthly")
data2 <- as.data.frame(NVDA)
...
# STOCK 3: AAPL Apple
# ============================================================
getSymbols("AAPL", from = "1999-11-01", to = "2025-11-02", periodicity = "monthly")
data3 <- as.data.frame(AAPL)
...
# STOCK 4: AMZN Amazon
# ============================================================
getSymbols("AMZN", from = "1999-11-01", to = "2025-11-02", periodicity = "monthly")
...
# ============================================================
# STOCK 5: BRK.A Berkshire Hathaway
# ============================================================
data5 <- getSymbols("BRK-A", 
                    from = "1999-11-01", 
                    to = "2025-11-02", 
                    periodicity = "monthly",
                    auto.assign = FALSE)
...
wealth_long_real <- wealth_data %>%
  ...
  pivot_longer(
    cols = c("Wealth_GMVP_Gross_Real", "Wealth_GMVP_Net_Real", "Wealth_MV_Gross_Real", "Wealth_MV_Net_Real"),
    names_to = "Portfolio",
    values_to = "Wealth"
  ) %>%
  mutate(
...
ggplot(subset(wealth_long_real, Group == "GMVP"), 
       aes(x = Date, y = Wealth, color = Type, linetype = Type)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = gmvp_colors, 
                     labels = c("Gross" = "GMVP gross", "Net" = "GMVP net")) +
  ...
  scale_x_date(date_breaks = "2 years", 
               date_labels = "%Y-%m",
               limits = c(as.Date("2004-11-01"), as.Date("2024-11-01"))) +
  labs(title = "Wealth GMVP: Gross vs Net (Real - Inflation-Adjusted)", 
       x = "Date (November of each year)", 
...
ggplot(subset(wealth_long_real, Group == "MV"), 
       aes(x = Date, y = Wealth, color = Type, linetype = Type)) +
  geom_line(size = 1.2) +
  scale_color_manual(values = mv_colors, 
                     labels = c("Gross" = "MV gross", "Net" = "MV net")) +
  ...
  scale_x_date(date_breaks = "2 years", 
               date_labels = "%Y-%m",
               limits = c(as.Date("2004-11-01"), as.Date("2024-11-01"))) +
  labs(title = "Wealth MV: Gross vs Net (Real - Inflation-Adjusted)", 
       x = "Date (November of each year)", 
       y = "Wealth (2004 dollars, initial = 1)", 
       color = "Type", linetype = "Type") +
  theme_minimal() +
