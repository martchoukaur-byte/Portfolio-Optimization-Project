...
# ============================================================
# STOCK 1: XOM ExxonMobil
# ============================================================
getSymbols("XOM", from = "1999-12-01", to = "2025-11-02", periodicity = "monthly")
data <- as.data.frame(XOM)

data <- data %>%
  mutate(
    returns = (XOM.Adjusted / lag(XOM.Adjusted) - 1) * 100,
    compound_growth = XOM.Adjusted / XOM.Adjusted[1]
  )

prices <- na.omit(data$XOM.Adjusted)
returns <- na.omit(data$returns)
std_dev <- sd(returns)

View(data)
# ============================================================
# STOCK 2: MSFT Microsoft
# ============================================================
getSymbols("MSFT", from = "1999-12-01", to = "2025-11-02", periodicity = "monthly")
data2 <- as.data.frame(MSFT)


data2 <- data2 %>%
  mutate(
    returns = (MSFT.Adjusted / lag(MSFT.Adjusted) - 1) * 100,
    compound_growth = MSFT.Adjusted / MSFT.Adjusted[1]
  )

prices2 <- data2$MSFT.Adjusted
...
# STOCK 3: C Citigroup
# ============================================================
getSymbols("C", from = "1999-12-01", to = "2025-11-02", periodicity = "monthly")
data3 <- as.data.frame(C)

data3 <- data3 %>%
  mutate(
    returns = (C.Adjusted / lag(C.Adjusted) - 1) * 100,
    compound_growth = C.Adjusted / C.Adjusted[1]
  )

prices3 <- data3$C.Adjusted
returns3 <- na.omit(data3$returns)
std_dev3 <- sd(returns3)

View(data3)
# ============================================================
# STOCK 4: GE General Electric
# ============================================================
getSymbols("GE", from = "1999-12-01", to = "2025-11-02", periodicity = "monthly")
data4 <- as.data.frame(GE)

data4 <- data4 %>%
  mutate(
    returns = (GE.Adjusted / lag(GE.Adjusted) - 1) * 100,
    compound_growth = GE.Adjusted / GE.Adjusted[1]
  )

prices4 <- data4$GE.Adjusted
...
# STOCK 5: WMT Walmart
# ============================================================
getSymbols("WMT", from = "1999-12-01", to = "2025-11-02", periodicity = "monthly")
data5 <- as.data.frame(WMT)

data5 <- data5 %>%
  mutate(
    returns = (WMT.Adjusted / lag(WMT.Adjusted) - 1) * 100,
    compound_growth = WMT.Adjusted / WMT.Adjusted[1]
  )

prices5 <- data5$WMT.Adjusted
...
# ============================================================
mean_return1 <- (1 + (data$XOM.Adjusted[312] - data$XOM.Adjusted[1]) / data$XOM.Adjusted[1])^(1/312)
mean_return2 <- (1 + (data2$MSFT.Adjusted[312] - data2$MSFT.Adjusted[1]) / data2$MSFT.Adjusted[1])^(1/312)
mean_return3 <- (1 + (data3$C.Adjusted[312] - data3$C.Adjusted[1]) / data3$C.Adjusted[1])^(1/312)
mean_return4 <- (1 + (data4$GE.Adjusted[312] - data4$GE.Adjusted[1]) / data4$GE.Adjusted[1])^(1/312)
mean_return5 <- (1 + (data5$WMT.Adjusted[312] - data5$WMT.Adjusted[1]) / data5$WMT.Adjusted[1])^(1/312)

# ============================================================
# Risk-free rate (3-month Treasury)
# ============================================================
rf_3m <- getSymbols("DGS3MO", src = "FRED", auto.assign = FALSE)

# Convertir en dataframe AVANT de filter
rf_3m <- as.data.frame(rf_3m)
rf_3m$date <- rownames(rf_3m)
rownames(rf_3m) <- NULL
colnames(rf_3m) <- c("yield", "date")
...
# ============================================================
getSymbols("SPY", from = "1999-12-01", to= '2025-11-02', periodicity = "monthly")
sp500_monthly <- as.data.frame(SPY)

sp500_monthly <- sp500_monthly %>%
  mutate(
    returns = (SPY.Adjusted / lag(SPY.Adjusted) - 1) * 100,
    compound_growth = SPY.Adjusted / SPY.Adjusted[1]
  )

prices_sp500 <- sp500_monthly$SPY.Adjusted
...
rf_capm_150$yield <- as.numeric(as.character(rf_capm_150$yield))
mean_rf_capm_150 <- mean(rf_capm_150$yield, na.rm = TRUE)
mean_rf_capm_150
...
rf_capm_all$yield <- as.numeric(as.character(rf_capm_all$yield))
mean_rf_capm_all <- mean(rf_capm_all$yield, na.rm = TRUE)
mean_rf_capm_all
...
ggplot(subset(wealth_long, Group == "GMVP"), aes(x = Year, y = Wealth, color = Type, linetype = Type)) +
  ...
  scale_x_continuous(
    breaks = seq(0, max(wealth_long$Year), by = 5),
    labels = function(x) as.integer(2000 + x)
  ) +
  scale_y_continuous(
    labels = scales::comma,
    trans = "log10"  # Échelle logarithmique pour mieux voir les différences
  ) +
  labs(
    title = "Wealth GMVP: Gross vs Net (Real Returns)",
    x = "Year",
    y = "Wealth (Initial = 1, Log Scale)",
    color = "Type",
    linetype = "Type",
    caption = "Source: Backtest Analysis"
  ) +
  theme_minimal() +
  theme(
...
ggplot(subset(wealth_long, Group == "MV"), aes(x = Year, y = Wealth, color = Type, linetype = Type)) +
  ...
  labs(
    title = "Wealth MV: Gross vs Net (Real Returns)",
    x = "Year",
    y = "Wealth (Initial = 1, Log Scale)",
    color = "Type",
    linetype = "Type",
...
ggplot(wealth_long, aes(x = Year, y = Wealth, color = Portfolio_Display, linetype = Type)) +
  ...
  scale_linetype_manual(
    values = c("Gross" = "solid", "Net" = "dashed"),
    labels = c("Gross" = "Gross Returns", "Net" = "Net Returns")
  ) +
  scale_x_continuous(
    breaks = seq(0, max(wealth_long$Year), by = 5),
    labels = function(x) as.integer(2004 + x)
  ) +
  scale_y_continuous(
    labels = scales::comma,
    trans = "log10"
  ...
  labs(
    ...
    subtitle = "Cumulative wealth growth with transaction costs impact (log scale)",
    x = "Year",
    y = "Wealth (Initial = 1, Log Scale)",
    color = "Portfolio",
    linetype = "Return Type",
    caption = "GMVP = Global Minimum Variance Portfolio | MV = Market-Value Weighted Portfolio\nGross = Before transaction costs | Net = After transaction costs"
  ) +
  theme_minimal() +
  theme(
